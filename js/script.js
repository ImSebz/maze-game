...
function displayVictoryMess(moves) {
  document.getElementById("moves").innerHTML = `¡Te moviste ${moves} pasos para llegar!`;
    ...
    while (!isComp) {
      ...
      for (index = 0; index < dirs.length; index++) {
        ...
        if (nx >= 0 && nx < width && ny >= 0 && ny < height) {
          //Revisa si la celda ya ha sido visitada
          if (!mazeMap[nx][ny].visited) {
            //
            mazeMap[pos.x][pos.y][direction] = true;
            mazeMap[nx][ny][modDir[direction].o] = true;

            //Establece la nueva celda como la celda actual
            mazeMap[nx][ny].priorPos = pos;
            //Actualizar la posición actual a la nueva celda
            pos = {
              x: nx,
              y: ny
            };
            cellsVisited++;
            //Recursivamente llame a la siguiente celda
            move = true;
            break;
          }
        }
      ...
      if (!move) {
        //  Si falla el movimiento, retroceda a la celda anterior y Recall el método.
        //  mueve la posición actual a la celda anterior y Recall el método.
...
//Cuando se hace click al okBtn se crea un nuevo Maze
